...
def make_text(text, color, bgcolor, top, left, size=None):
    if size is None:
        text_rendering = BASICFONT.render(text, True, color, bgcolor)
    else:
        font = pygame.font.Font('freesansbold.ttf', size)
        text_rendering = font.render(text, True, color, bgcolor)
...
def puzzle(sreen):
    ...
    SOLVE_SURF = make_text(
        'Решено!', TEXT, BGCOLOR, win_width - 130, win_height - 470, size=25)
    ...
    while True:
        ...
        if slide_to:
            sliding_animation(
                main_board, slide_to, 'Щелкните по блоку или нажмите клавиши со '
                                      'стрелками, чтобы сдвинуть блок.', 8)
